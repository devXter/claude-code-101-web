...
const CMDS = {
  scaffold: `cd frontend && npm create vite@6 . -- --template react-ts && npm install && cd ..`,
  twInstall: `npm install -D tailwindcss @tailwindcss/vite`,
  viteConfig: `cat > vite.config.ts << 'EOF'\nimport { defineConfig } from 'vite'\nimport react from '@vitejs/plugin-react'\nimport tailwindcss from '@tailwindcss/vite'\n\nexport default defineConfig({\n  plugins: [react(), tailwindcss()],\n})\nEOF`,
      ...
      >
        ...
        >
          ...
          <button
            onClick={copy}
            style={{
              background: copied ? '#28c84033' : '#ffffff11',
              border: `1px solid ${copied ? '#28c840' : '#ffffff22'}`,
              borderRadius: 6,
              padding: '4px 14px',
              cursor: 'pointer',
              fontSize: 11,
...
const SectionTitle = ({ icon, children, sub }) => (
  <div style={{ marginBottom: 48 }}>
    ...
    {sub && (
      <FadeIn delay={0.15}>
        <p style={{ fontSize: 16, color: C.muted, maxWidth: 720, lineHeight: 1.7, margin: 0 }}>{sub}</p>
      </FadeIn>
    )}
  </div>
);
...
const Section = ({ id, children, style = {}, hero = false }) => (
  <section
    id={id}
    style={{
      minHeight: hero ? '100vh' : 'auto',
      padding: hero ? '100px max(24px, 5vw)' : '80px max(24px, 5vw)',
      display: 'flex',
...
export default function App() {
  ...
  return (
    <div
      style={{ background: C.bg, color: C.text, fontFamily: "'DM Sans', 'Segoe UI', sans-serif", overflowX: 'hidden' }}
    >
      <style>{`
      ...
      >
        {nav.map((n) => (
          <button
            ...
            title={n.label}
            style={{
              width: active === n.id ? 12 : 8,
              height: active === n.id ? 12 : 8,
              borderRadius: '50%',
              border: 'none',
              cursor: 'pointer',
              background: active === n.id ? C.primary : `${C.muted}44`,
              transition: 'all 0.3s',
            }}
          />
        ))}
      ...
      {active !== 'hero' && (
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = `${C.teal}33`;
            e.currentTarget.style.borderColor = C.teal;
            e.currentTarget.style.transform = 'translateY(-2px)';
            e.currentTarget.style.boxShadow = `0 8px 30px ${C.teal}28`;
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = `${C.teal}22`;
            e.currentTarget.style.borderColor = `${C.teal}88`;
            e.currentTarget.style.transform = 'translateY(0)';
            e.currentTarget.style.boxShadow = `0 4px 20px ${C.teal}18`;
          }}
        >
          ↑
        </button>
      ...
      <header
        style={{
          ...
          background: `${C.bg}dd`,
          backdropFilter: 'blur(12px)',
          borderBottom: `1px solid ${C.card}22`,
        }}
      >
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, color: C.primary, fontWeight: 600 }}>
          {'>'}_claude_code_101
        </span>
      ...
      >
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: `radial-gradient(circle at 30% 40%, ${C.primary}11 0%, transparent 60%), radial-gradient(circle at 80% 70%, ${C.teal}08 0%, transparent 50%)`,
          }}
        />
        <div
          style={{
            ...
            bottom: 0,
            width: 4,
            background: `linear-gradient(to bottom, ${C.primary}, ${C.teal})`,
          }}
        />
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <FadeIn delay={0.5}>
            <p style={{ fontSize: 16, color: C.muted, maxWidth: 560, lineHeight: 1.7 }}>
              Descubre cómo las extensiones de Claude Code transforman tu flujo de desarrollo. De Skills a Hooks, de MCP
              a Subagents — todo lo que necesitas para empezar.
            </p>
          </FadeIn>
          <FadeIn delay={0.65}>
      ...
      <Section id="what">
        <SectionTitle
          icon="⌨️"
          sub="Una herramienta de desarrollo agéntico. Lee tu codebase completo, edita archivos, ejecuta comandos e integra con tus herramientas de desarrollo. Disponible en terminal, IDE, desktop app y browser."
        >
          ¿Qué es Claude Code?
        </SectionTitle>
        <div
        ...
        >
          {[
            {
              icon: '🔍',
              title: 'Recopilar Contexto',
              desc: 'Lee archivos, busca en el repo, analiza dependencias',
              color: C.teal,
            },
            {
              icon: '⚡',
              title: 'Ejecutar Acción',
              desc: 'Edita código, ejecuta comandos, crea archivos',
              color: C.accent,
            },
            {
              icon: '✓',
              title: 'Verificar Resultados',
              desc: 'Corre tests, valida builds, verifica el output',
              color: C.success,
            },
          ].map((p, i) => (
            <Card
              key={i}
              delay={i * 0.15}
              style={{ borderTop: `3px solid ${p.color}`, textAlign: 'center', padding: 32 }}
            >
              <div
                style={{
                  fontSize: 40,
      ...
      <Section id="arch">
        <SectionTitle
          icon="🚀"
          sub="Las herramientas integradas son la base. Las extensiones forman capas encima del bucle agéntico principal."
        >
        ...
        <FadeIn>
          ...
          >
            ...
            <div
              className="agentic-loop-grid"
              style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 32, alignItems: 'center' }}
            >
              {/* Columna izquierda: Diagrama SVG circular */}
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <svg width="300" height="300" viewBox="0 0 300 300" style={{ overflow: 'visible' }}>
                  {/* Anillo punteado rotatorio — centro (150,160), radio 100 */}
                  <g style={{ transformOrigin: '150px 160px', animation: 'orbitSpin 30s linear infinite' }}>
                    <circle
                      cx="150"
                      cy="160"
                      r="100"
                  ...
                  <defs>
                    <marker id="arrowTeal" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
                      <polygon points="0 0, 8 3, 0 6" fill={C.teal} />
                    </marker>
                    ...
                    <marker id="arrowSuccess" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
                      <polygon points="0 0, 8 3, 0 6" fill={C.success} />
                    </marker>
                  </defs>

                  {/* Recopilar (150,60) → Ejecutar (237,210): arco circular horario */}
                  <path
                    d="M 182 60 Q 250 103 253 182"
                    fill="none"
                    stroke={C.teal}
                    strokeWidth="2"
                    markerEnd="url(#arrowTeal)"
                  />
                  {/* Ejecutar (237,210) → Verificar (63,210): arco circular horario */}
                  <path
                    d="M 221 238 Q 150 275 79 238"
                    fill="none"
                    stroke={C.accent}
                    strokeWidth="2"
                    markerEnd="url(#arrowAccent)"
                  />
                  {/* Verificar (63,210) → Recopilar (150,60): arco circular horario */}
                  <path
                    d="M 47 182 Q 50 103 118 60"
                    fill="none"
                    stroke={C.success}
                    strokeWidth="2"
                  ...
                  {/* Nodo: Recopilar (arriba) — cx=150, cy=60 */}
                  <g>
                    <circle cx="150" cy="60" r="32" fill={C.dark} stroke={C.teal} strokeWidth="2.5" />
                    <text x="150" y="56" textAnchor="middle" fill={C.teal} fontSize="16">
                      🔍
                    </text>
                    <text
                      x="150"
                  ...
                  {/* Nodo: Ejecutar (derecha-abajo) — cx=237, cy=210 */}
                  <g>
                    <circle cx="237" cy="210" r="32" fill={C.dark} stroke={C.accent} strokeWidth="2.5" />
                    <text x="237" y="206" textAnchor="middle" fill={C.accent} fontSize="16">
                      ⚡
                    </text>
                    <text
                      x="237"
                  ...
                  {/* Nodo: Verificar (izquierda-abajo) — cx=63, cy=210 */}
                  <g>
                    <circle cx="63" cy="210" r="32" fill={C.dark} stroke={C.success} strokeWidth="2.5" />
                    <text x="63" y="206" textAnchor="middle" fill={C.success} fontSize="16">
                      ✓
                    </text>
                    <text
                      x="63"
                ...
                {[
                  {
                    icon: '🔍',
                    name: 'Recopilar',
                    desc: 'Lee archivos, busca en el repo, analiza dependencias y contexto',
                    color: C.teal,
                  },
                  {
                    icon: '⚡',
                    name: 'Ejecutar',
                    desc: 'Edita código, ejecuta comandos, crea archivos y aplica cambios',
                    color: C.accent,
                  },
                  {
                    icon: '✓',
                    name: 'Verificar',
                    desc: 'Corre tests, valida builds, verifica el output y corrige',
                    color: C.success,
                  },
                ].map((phase, i) => (
                  <div
                  ...
                  >
                    ...
                    <div>
                      <div style={{ color: phase.color, fontWeight: 700, fontSize: 15, marginBottom: 2 }}>
                        {phase.name}
                      </div>
                      <div style={{ color: C.muted, fontSize: 13, lineHeight: 1.5 }}>{phase.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Responsive: apilar columnas en mobile */}
            <style>{`
              @media (max-width: 720px) {
                .agentic-loop-grid { grid-template-columns: 1fr !important; }
              }
            `}</style>
          </div>
        </FadeIn>
          ...
          {[
            ...
            <FadeIn key={i} delay={i * 0.1}>
              ...
              >
                ...
                <code
                  style={{ fontSize: 11, color: ext.color, background: C.dark, padding: '4px 8px', borderRadius: 4 }}
                >
                  {ext.ex}
                </code>
              </div>
      ...
      <Section id="context">
        <SectionTitle
          icon="📊"
          sub="Cada feature se carga en un momento diferente de tu sesión. Entender esto es clave para optimizar el uso."
        >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {[
            ...
            },
            {
              title: 'On Use',
              color: C.accent,
              items: [{ name: 'Skills', desc: 'Contenido completo al invocar', style: 'dashed' }],
            },
            {
              title: 'Isolated',
              color: C.success,
                  ...
                  {col.items.map((item, ii) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        padding: 16,
                        borderLeft: `3px ${item.style} ${col.color}`,
                      }}
                    >
      ...
      <Section id="claude-md">
        <SectionTitle
          icon="📄"
          sub="Archivo markdown inyectado en el system prompt de cada sesión. Define convenciones, comandos y reglas. Se genera con /init."
        >
...
- Conventional Commits`}</CodeBlock>
          <div>
            <FadeIn delay={0.2}>
              <h3 style={{ color: C.accent, fontSize: 18, marginBottom: 20, fontWeight: 700 }}>Jerarquía + Rules</h3>
            </FadeIn>
            {[
            ...
            <FadeIn delay={0.8}>
              <div style={{ background: C.highlight, borderRadius: 8, padding: 16, marginTop: 20 }}>
                <p style={{ color: C.accent, fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
                  Pro tip: Claude Code te enseña a usarlo.
                </p>
                <p style={{ color: C.muted, fontSize: 12 }}>
                  <code style={{ color: C.success }}>/init</code> crea CLAUDE.md ·{' '}
                  <code style={{ color: C.teal }}>/agents</code> configura subagents ·{' '}
                  <code style={{ color: C.primary }}>/doctor</code> diagnostica
                </p>
              </div>
            </FadeIn>
          </div>
      ...
      <Section id="skills">
        <SectionTitle
          icon="🧠"
          sub="Guías en markdown que enseñan a Claude cómo manejar tareas específicas. Invocadas por el usuario o automáticamente."
        >
...
5. Commit con mensaje conventional`}</CodeBlock>
          <div>
            <FadeIn delay={0.2}>
              <h3 style={{ color: C.accent, fontSize: 18, marginBottom: 20, fontWeight: 700 }}>Características</h3>
            </FadeIn>
            {[
      ...
      <Section id="mcp">
        <SectionTitle
          icon="🔌"
          sub="Estándar abierto (Linux Foundation) que conecta Claude con servicios externos. 300+ integraciones."
        >
      ...
      <Section id="hooks">
        <SectionTitle
          icon="🔧"
          sub="Comandos shell en puntos del ciclo de vida. Garantías que el LLM solo no puede dar. Se ejecutan FUERA del contexto."
        >
        ...
        <FadeIn>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 32, justifyContent: 'center' }}>
            {[
              { name: 'SessionStart', c: C.teal },
              { name: 'UserPromptSubmit', c: C.primary },
              { name: 'PreToolUse', c: C.accent },
              { name: 'PostToolUse', c: C.success },
              { name: 'Stop', c: C.purple },
              { name: 'SubagentStop', c: C.muted },
            ].map((h, i) => (
              <div
                key={i}
                style={{
                  background: C.card,
                  borderTop: `3px solid ${h.c}`,
                  borderRadius: 8,
                  padding: '10px 16px',
                  textAlign: 'center',
                }}
        ...
        <FadeIn delay={0.2}>
          <h3 style={{ color: C.accent, fontSize: 16, marginBottom: 12, fontWeight: 600 }}>
  ...
  "hooks": {
    ...
    "PreToolUse": [{
      ...
      <Section id="features">
        ...
        >
          {[
            {
              title: 'Plan Mode',
              desc: 'Claude crea un plan detallado que puedes revisar antes de codificar.',
              color: C.primary,
            },
            {
              title: 'Background Agents',
              desc: 'Envía tareas al background con Ctrl+B y sigue trabajando.',
              color: C.teal,
            },
            {
              title: 'Agent Teams',
              desc: 'Múltiples instancias en paralelo en diferentes partes del proyecto.',
              color: C.accent,
            },
            {
              title: 'IDE Integration',
              desc: 'VS Code, JetBrains, Desktop App. Diffs inline, checkpoints.',
              color: C.success,
            },
            {
              title: 'Git Operations',
              desc: 'Commits, branches, PRs via gh CLI. Conventional commits automáticos.',
              color: C.purple,
            },
      ...
      <Section id="plugins">
        <SectionTitle
          icon="⚙️"
          sub="Un plugin agrupa Skills, Hooks, Subagents y servidores MCP en una única unidad instalable y distribuible."
        >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          <div>
            <FadeIn>
              <h3 style={{ color: C.accent, fontSize: 16, marginBottom: 16, fontWeight: 700 }}>
            ...
            {[
              ...
              <FadeIn key={i} delay={i * 0.1}>
                ...
                >
                  <div>
                    <div style={{ color: p.color, fontWeight: 600, fontSize: 14 }}>{p.name}</div>
          ...
          <div>
            <FadeIn delay={0.2}>
              <h3 style={{ color: C.accent, fontSize: 16, marginBottom: 16, fontWeight: 700 }}>Distribución</h3>
            </FadeIn>
            <Card delay={0.3}>
              <div style={{ color: C.white, fontWeight: 600, marginBottom: 8 }}>Marketplaces</div>
              <p style={{ color: C.muted, fontSize: 13 }}>
                Instala plugins de la comunidad o crea tu propio marketplace interno.
              </p>
            </Card>
            <div style={{ height: 12 }} />
            <Card delay={0.4}>
              ...
              <p style={{ color: C.muted, fontSize: 13, marginBottom: 8 }}>
                Skills de plugin usan namespaces para coexistir:
              </p>
              <code
                style={{
                  color: C.accent,
      ...
      <Section id="costs">
        <SectionTitle
          icon="📈"
          sub="Cada extensión consume contexto de forma diferente. Entender estos costos te ayuda a construir una configuración efectiva."
        >
        ...
        <FadeIn>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 4px', fontSize: 14 }}>
              <thead>
                <tr>
                  {['Feature', 'Cuándo se carga', 'Qué se carga', 'Costo de contexto', 'Nivel'].map((h, i) => (
                    <th
                      key={i}
                      style={{
                        padding: '12px 16px',
              ...
              <tbody>
                {[
                  {
                    feat: 'CLAUDE.md',
                    when: 'Inicio de sesión',
                    what: 'Contenido completo',
                    cost: 'Cada solicitud',
                    level: 'Alto',
                    color: C.primary,
                    lc: C.primary,
                  },
                  {
                    feat: 'Skills',
                    when: 'Inicio + al usar',
                    what: 'Descripciones → contenido',
                    cost: 'Bajo (descripciones)',
                    level: 'Bajo → Alto',
                    color: C.purple,
                    lc: C.accent,
                  },
                  ...
                  },
                  {
                    feat: 'Subagents',
                    when: 'Al generarse',
                    what: 'Contexto fresco',
                    cost: 'Aislado del principal',
                    level: 'Cero',
                    color: C.success,
                    lc: C.success,
                  },
      ...
      <Section id="patterns">
        <SectionTitle
          icon="🔗"
          sub="Cada extensión resuelve un problema diferente. Las configuraciones reales las combinan."
        >
        ...
        {[
          {
            pattern: 'Skill + MCP',
            color: C.purple,
            how: 'MCP provee la conexión; la Skill enseña cómo usarla bien',
            ex: 'MCP conecta tu DB. La Skill documenta el esquema y patrones de query.',
          },
          {
            pattern: 'Skill + Subagent',
            color: C.success,
            how: 'La Skill genera subagents para trabajo paralelo aislado',
            ex: '/audit lanza subagents de seguridad, rendimiento y estilo.',
          },
          {
            pattern: 'CLAUDE.md + Skills',
            color: C.primary,
            how: 'CLAUDE.md = reglas siempre activas. Skills = referencia bajo demanda',
            ex: 'CLAUDE.md dice "sigue convenciones de API". La Skill contiene la guía completa.',
          },
          {
            pattern: 'Hook + MCP',
            color: C.teal,
            how: 'El Hook dispara acciones externas vía MCP automáticamente',
            ex: 'Hook post-edición envía notificación a Slack al modificar archivos críticos.',
          },
        ].map((c, i) => (
      ...
      <Section id="bench">
        <SectionTitle
          icon="📊"
          sub="SWE-bench Verified toma issues reales de repos open source en GitHub y mide qué % un modelo resuelve solo. Datos: Marzo 2026."
        >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32 }}>
          <FadeIn>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { name: 'Claude Opus 4.5', val: 80.9 },
                { name: 'Claude Opus 4.6', val: 80.8 },
                { name: 'Gemini 3.1 Pro', val: 80.6 },
                { name: 'MiniMax M2.5', val: 80.2 },
                { name: 'GPT-5.2', val: 80.0 },
                { name: 'Claude Sonnet 4.6', val: 79.6 },
              ].map((m, i) => {
                const pct = ((m.val - 78) / 4) * 100;
                return <BarRow key={i} name={m.name} val={m.val} pct={pct} delay={i * 0.1} />;
              })}
            </div>
          </FadeIn>
          <div>
            <Card delay={0.3} accent={C.accent}>
              ...
              <p style={{ color: C.text, fontSize: 13, lineHeight: 1.7, marginBottom: 16 }}>
                Los 6 mejores modelos están a menos de 1.3 puntos entre sí. La diferencia real ya no es el modelo — es
                cómo lo configuras y usas.
              </p>
      ...
      {/* ═══ PRIVACY ═══ */}
      <Section id="privacy">
        <SectionTitle icon="🛡️" sub="">
          Privacidad y Propiedad del Código
        </SectionTitle>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          <div>
            <FadeIn>
              <h3 style={{ color: C.accent, fontSize: 18, marginBottom: 20, fontWeight: 700 }}>
                ¿Qué Pasa con Tu Código?
              </h3>
            </FadeIn>
            {[
              'Solo archivos que Claude lee se envían',
              'Transporte encriptado TLS',
              'Ejecución de código es LOCAL',
              'Sandboxing aísla filesystem + red',
            ].map((t, i) => (
              <FadeIn key={i} delay={i * 0.1}>
            ...
            <FadeIn delay={0.5}>
              <h4 style={{ color: C.accent, fontSize: 14, marginTop: 24, marginBottom: 12, fontWeight: 700 }}>
                ¿Se usa tu código para entrenar modelos?
              </h4>
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 4px', fontSize: 13 }}>
                <thead>
                  <tr>
                    {['Plan', 'Retención', '¿Entrena?'].map((h, i) => (
                      <th
                        key={i}
                        style={{
                          padding: '8px 12px',
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ background: i % 2 === 0 ? C.dark : C.card }}>
                      ...
                      <td style={{ padding: '8px 12px', color: r.c, fontFamily: "'JetBrains Mono', monospace" }}>
                        {r.ret}
                      </td>
          ...
          <div>
            <FadeIn>
              <h3 style={{ color: C.accent, fontSize: 18, marginBottom: 16, fontWeight: 700 }}>
            ...
            <FadeIn delay={0.15}>
              ...
              >
                <code style={{ color: C.accent, fontSize: 13 }}>
                  Términos de Servicio de Anthropic, Sección 4(a):{'\n'}"you own all Outputs"
                </code>
              </div>
            </FadeIn>
            <FadeIn delay={0.25}>
              <h4 style={{ color: C.success, fontSize: 14, marginBottom: 12, fontWeight: 700 }}>
                Lo que dicen los Términos de Servicio:
              </h4>
            </FadeIn>
            {[
              'Anthropic asigna todos los derechos al usuario',
              'Planes comerciales incluyen indemnización',
              'Tu código NO se incorpora al modelo (tú decides)',
            ].map((t, i) => (
            ...
            <FadeIn delay={0.6}>
              <h4 style={{ color: C.accent, fontSize: 14, marginTop: 20, marginBottom: 12, fontWeight: 700 }}>
                Zona gris legal:
              </h4>
            </FadeIn>
            {[
              'Copyright requiere autoría humana (US Copyright Office)',
              'Código 100% IA sin input humano puede no ser protegible',
              'Usa Claude como asistente, documenta tu proceso creativo',
            ].map((t, i) => (
        ...
        <FadeIn delay={0.9}>
          <div style={{ background: C.highlight, borderRadius: 8, padding: 16, marginTop: 24, textAlign: 'center' }}>
            <p style={{ color: C.accent, fontWeight: 700, fontSize: 14 }}>
              Anthropic NO usa tus datos arbitrariamente. Siempre es decisión del usuario. Settings {'>'} Privacy para
              configurar.
            </p>
          </div>
        </FadeIn>
      ...
      <Section id="best">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          <div>
            <FadeIn>
              <h3 style={{ color: C.success, fontSize: 20, marginBottom: 20, fontWeight: 800 }}>HACER</h3>
            </FadeIn>
            {[
              'Usar CLAUDE.md con convenciones del equipo',
              'Activar Plan Mode para tareas complejas',
              'Revisar cada output — no aceptar ciegamente',
              'Escribir prompts específicos con contexto',
              'Mantener tests como red de seguridad',
              'Usar Skills compartidas via git',
              'Hooks para formateo y validación automática',
            ].map((t, i) => (
          ...
          <div>
            <FadeIn>
              <h3 style={{ color: C.danger, fontSize: 20, marginBottom: 20, fontWeight: 800 }}>EVITAR</h3>
            </FadeIn>
            {[
              'Refactorizar legacy masivo sin tests',
              'Prompts vagos: "arregla esto"',
              'Ignorar el Plan Mode en features grandes',
              'Dejar que Claude haga commits sin revisar',
              'Usar --dangerously-skip en producción',
              'Confiar en IA para lógica de negocio crítica',
              'Olvidar documentar en CLAUDE.md',
            ].map((t, i) => (
              <FadeIn key={i} delay={i * 0.08}>
                <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
        ...
        <FadeIn delay={0.7}>
          <div style={{ textAlign: 'center', marginTop: 40, padding: '20px 0', borderTop: `2px solid ${C.primary}33` }}>
            <p style={{ color: C.accent, fontStyle: 'italic', fontSize: 16 }}>
              "La IA no reemplaza al desarrollador. Potencia al que sabe usarla con criterio técnico."
            </p>
          </div>
        </FadeIn>
      </Section>
      ...
      <Section id="close" hero={true} style={{ textAlign: 'center' }}>
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: `radial-gradient(circle at 50% 50%, ${C.primary}11 0%, transparent 60%)`,
          }}
        />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <FadeIn>
          ...
          <FadeIn delay={0.2}>
            <p style={{ fontSize: 22, color: C.accent, marginBottom: 48 }}>Live Demo</p>
          </FadeIn>
          <FadeIn delay={0.4}>
            <button
              onClick={() => scrollTo('demo')}
            ...
            <div style={{ marginTop: 48, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ color: C.accent, fontSize: 12, fontWeight: 600, minWidth: 80, textAlign: 'right' }}>
                    {r.label}
                  </span>
      ...
      <Section id="demo">
        <SectionTitle
          icon="🎯"
          sub="ContactBook App — Gestión de contactos con React + NestJS. El foco es cómo Claude Code con sus extensiones potencia el desarrollo."
        >
        ...
        >
          {[
            {
              phase: 'Fase 1',
              title: 'Proyecto + CLAUDE.md',
              time: '3 min',
              desc: 'Mostrar extensiones configuradas',
              color: C.primary,
            },
            {
              phase: 'Fase 2',
              ...
              time: '10 min',
              desc: 'NestJS + Plan Mode + Hook Prettier',
              color: C.teal,
            },
            {
              phase: 'Fase 3',
              title: 'Tests BDD con Skill',
              time: '5 min',
              desc: '/test-bdd ContactsService',
              color: C.purple,
            },
            {
              phase: 'Fase 4',
              title: 'Frontend con Subagent',
              time: '7 min',
              desc: 'React + Tailwind en contexto aislado',
              color: C.success,
            },
            {
              phase: 'Fase 5',
              title: 'MCP GitHub → PR',
              time: '5 min',
              desc: 'Issue #1 → implementar → PR automático',
              color: C.accent,
            },
          ].map((p, i) => (
        ...
        <FadeIn delay={0.7}>
          <div style={{ textAlign: 'center', marginTop: 40, padding: 24, background: C.highlight, borderRadius: 12 }}>
            ...
            <p style={{ color: C.muted, fontSize: 13 }}>
              Stack: React 19 + Build Tool 7 + TailwindCSS (frontend) | NestJS 11 + Hexagonal Architecture (backend)
            </p>
          </div>
        </FadeIn>
      </Section>
      ...
      <Section id="guide">
        <SectionTitle
          icon="📋"
          sub="Sigue esta guía paso a paso para replicar el Live Demo en tu máquina. Cada paso indica qué herramienta usar."
        >
          Guía Paso a Paso
        </SectionTitle>
...
git push origin main`}</CopyBlock>
        ...
        <GuideStep
          number="3"
          title="Configurar con Claude Code"
        ...
        <FadeIn>
          <h2
            style={{
              ...
              fontWeight: 700,
              margin: '48px 0 24px',
              borderTop: `2px solid ${C.card}`,
              paddingTop: 32,
            }}
          >
            🎬 Ejecutar el Live Demo
          </h2>
        </FadeIn>
...
claude --dangerously-skip-permissions`}</CopyBlock>
        ...
        <GuideStep
          number="F2"
          title="Backend Hexagonal"
          badge={{ icon: '🧠', label: 'CLAUDE.md + Hook' }}
          badgeColor={C.accent}
        >
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 12 }}>
            Presiona <code style={{ color: C.accent }}>Shift+Tab</code> para activar Plan Mode, luego:
          </p>
...
8. Abre un PR que referencie el issue #1`}</CopyBlock>
        ...
        <FadeIn>
          <div
            style={{ background: C.dark, borderRadius: 12, padding: 32, border: `1px solid ${C.card}`, marginTop: 48 }}
          >
